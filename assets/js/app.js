...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Перебираем карточки. Последняя не будет кликабельной.
  cards.forEach((card, index) => {
    if (index === cards.length - 1) return; // пропускаем последнюю карточку

    card.addEventListener("click", function () {
      // Добавляем класс для анимации ухода карточки вниз
      card.classList.add("dismiss");

      // После завершения анимации (0.5 сек по CSS) скрываем карточку
      card.addEventListener(
        "transitionend",
        function () {
          card.style.display = "none";
          // Если есть следующая карточка, убираем у неё поворот
